...
const createRoom = onCall(options, async (request) => {
  const uid = uidOf(request); const rid = requestId(request); const displayName = text(request.data?.displayName, 20, '表示名'); const humanPlayerCount = Number(request.data?.humanPlayerCount ?? request.data?.playerCount); if (![2, 3, 4].includes(humanPlayerCount)) fail('invalid-argument', '人間プレイヤー数は2〜4人です。');
  const payloadHash = hash('create', { displayName, playerCount: humanPlayerCount }); const action = actionRef(uid, rid); const prior = await action.get(); const invite = createInviteCode(uid, rid, key()); if (prior.exists) return { ...replay(prior.data(), payloadHash), inviteCode: invite.code };
  const id = crypto.randomUUID(); const ref = roomRef(id); const result = { roomId: id, stateVersion: 1 }; const expiresAt = expiry();
  await db.runTransaction(async (tx) => { if ((await tx.get(action)).exists) return; const locatorRef = db.collection('deepMiningAgreementRoomLocators').doc(invite.locator); if ((await tx.get(locatorRef)).exists) fail('already-exists', '招待コードが競合しました。'); const member = { uid, displayName, seatId: 'seat1', joinedAt: Timestamp.now(), leftAt: null }; tx.set(ref, { id, status: 'lobby', hostUid: uid, humanPlayerCount, stateVersion: 1, gameId: null, members: [member], expiresAt }); tx.set(memberRef(ref, uid), { ...member, expiresAt }); tx.set(locatorRef, { roomId: id, inviteMac: mac(invite.locator, invite.secret, key()), expiresAt }); tx.set(action, { payloadHash, result, expiresAt }); });
