// もふもふ大集合！ 人間3〜6人オンライン版 Phase E: Firebase接続先の解決。
//
// 既存2人＋こはる版（../firebase-config.js）と同じproject・同じ値・同じApp Checkを使う。
// 3〜6人版は online/multi/ という別ディレクトリのため、値の取り違えを防ぐ目的でこのファイルへ自前コピーを持つ
// （既存ファイルをimportして共有すると、既存ページの変更が3〜6人版へ波及する）。
...
// Emulatorは firebase.mofumofu-multi-emulator.json のポートへ合わせる（firestore 8181 / database 9000 / functions 5102）。
export function resolveEnvironment() {
  if (localHosts.has(host)) return {
    ...
    emulator: { authPort: 9199, firestorePort: 8181, databasePort: 9000, functionsPort: 5102 },
    appCheck: { debug: true },
  };
