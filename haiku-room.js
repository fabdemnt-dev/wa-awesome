...
function updatePhaseStatus(data) {
  ...
  if (data?.status === 'playing') {
    ...
    // roundPlayerUids / participantUidsを優先して解決し、途中の役割変更でも履歴を失わない。
    const players = data.players || [];
    const participantUids = data.participantUids || {};
    const roundEntries = getRoundParticipantEntries(data);
    const uidFor = name => roundEntries.find((entry) => entry.name === name)?.uid
      || Object.entries(participantUids).find(([, value]) => value === name)?.[0];
    const phrases = data.phrases || {};
    const revealedPhrases = data.revealedPhrases || {};
    const phraseKeyFor = name => {
      const uid = uidFor(name);
      return uid && phrases[uid] !== undefined ? uid : name;
    };
...
function updateSubmissionStatus(data) {
  const players = data?.status === 'playing' ? (data?.players || []) : (data?.players || []);
  const participantUids = data?.participantUids || {};
...
function updateScoreHistory(data) {
  ...
  el.innerHTML = `
    <details class="score-history-details"${wasOpen ? ' open' : ''}>
      ...
      ${roundHtml}
      <button type="button" class="score-history-close" onclick="const details=this.closest('details'); if (details) details.open=false; document.getElementById('score-history')?.scrollIntoView({ behavior: 'smooth', block: 'start' });">閉じて上へ戻る</button>
    </details>
  `;
}
